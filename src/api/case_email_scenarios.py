...
def get_email_data(scenarios_name):
    sql_text ="SELECT  e.id,e.email_subject,e.msg_template,e.email_to,e.email_cc,e.email_bcc\
                ...
                inner join group_scenarios as gs\
                on tg.id = gs.task_group_id\
                inner join task_scenarios as ts\
                on gs.task_scenarios_id = ts.id\
...
def send_email(scenarios_name):
    bl = False
    try:
        ls_kpi,ls_failed = get_email_data(scenarios_name)
